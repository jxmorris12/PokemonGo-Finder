...
def transform_from_wgs_to_gcj(wgs_loc):
    ...
    if is_location_out_of_china(wgs_loc):
        ...
        adjust_lat = transform_lat(
                wgs_loc.longitude - 105, wgs_loc.latitude - 35.0)
        adjust_lon = transform_long(
                wgs_loc.longitude - 105, wgs_loc.latitude - 35.0)
        ...
        adjust_lat =\
            (adjust_lat * 180.0) / ((a * (1 - ee)) / (magic * sqrt_magic) * pi)
...
def is_location_out_of_china(wgs):
    if wgs.longitude < 72.004\
            or wgs.longitude > 137.8347\
            or wgs.latitude < 0.8293\
            or wgs.latitude > 55.8271:
        return True
    return False
...
def transform_lat(x, y):
    lat = -100.0 + 2.0 * x + 3.0 * y\
          + 0.2 * y * y + 0.1 * x * y + 0.2 * sqrt(abs(x))
